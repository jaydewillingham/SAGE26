...
int32_t initialize_hdf5_galaxy_files(const int filenr, struct save_info *save_info, const struct params *run_params)
{
    ...
    for(int32_t snap_idx = 0; snap_idx < run_params->NumSnapOutputs; snap_idx++) {
        ...
        const char *bh_names[9] = {"BHAccretionType", "dt", "BHMassatAccretion", "BHEddingtonRateLimit", "BHMaxaccretionRate", "RadioModeBHaccretionMass", "InstabilityDrivenBHaccretionMass", "MergerDrivenBHaccretionMass", "BHMergerMass"};
        ...
        const char *bh_units[9] = {"unitless", "code_time","1.0e10Msun/h", "1.0e10 Msun/h", "1.0e10 Msun/h", "1.0e10 Msun/h", "1.0e10 Msun/h", "1.0e10 Msun/h", "1.0e10 Msun/h"};
        ...
        for(int cum_idx = 0; cum_idx < 9; cum_idx++) {
            ...
            CHECK_STATUS_AND_RETURN_ON_FAIL(prop_bh, (int32_t) prop_bh,
                                            "Could not create property list for BH history dataset %s", bh_names[cum_idx]);

            herr_t bh_status = H5Pset_chunk(prop_bh, 2, chunk_dims_bh);
            CHECK_STATUS_AND_RETURN_ON_FAIL(bh_status, (int32_t) bh_status,
                                            "Could not set chunking for BH history dataset %s", bh_names[cum_idx]);
            ...
            CHECK_STATUS_AND_RETURN_ON_FAIL(dataspace_bh, (int32_t) dataspace_bh,
                                            "Could not create dataspace for BH history dataset %s", bh_names[cum_idx]);
            ...
            CHECK_STATUS_AND_RETURN_ON_FAIL(dataset_bh, (int32_t) dataset_bh,
                                            "Could not create BH history dataset %s", bh_names[cum_idx]);

            CREATE_STRING_ATTRIBUTE(dataset_bh, "Description", bh_descriptions[cum_idx], MAX_STRING_LEN);
            CREATE_STRING_ATTRIBUTE(dataset_bh, "Units", bh_units[cum_idx], MAX_STRING_LEN);
            CREATE_SINGLE_ATTRIBUTE(dataset_bh, "NumSnapshots", run_params->SimMaxSnaps, H5T_NATIVE_INT);

            bh_status = H5Dclose(dataset_bh);
            CHECK_STATUS_AND_RETURN_ON_FAIL(bh_status, (int32_t) bh_status,
                                            "Failed to close BH history dataset %s", bh_names[cum_idx]);

            bh_status = H5Pclose(prop_bh);
            CHECK_STATUS_AND_RETURN_ON_FAIL(bh_status, (int32_t) bh_status,
                                            "Failed to close property list for BH history dataset %s", bh_names[cum_idx]);

            bh_status = H5Sclose(dataspace_bh);
            CHECK_STATUS_AND_RETURN_ON_FAIL(bh_status, (int32_t) bh_status,
                                            "Failed to close dataspace for BH history dataset %s", bh_names[cum_idx]);
        }
    ...
    for(int32_t snap_idx = 0; snap_idx < run_params->NumSnapOutputs; snap_idx++) {
        ...
        save_info->buffer_output_gals[snap_idx].BHAccretionType = malloc(save_info->buffer_size * run_params->SimMaxSnaps * sizeof(int32_t));
        save_info->buffer_output_gals[snap_idx].dt = malloc(save_info->buffer_size * run_params->SimMaxSnaps * sizeof(float));
        ...
        if(save_info->buffer_output_gals[snap_idx].BHEddingtonRateLimit == NULL ||
           ...
           save_info->buffer_output_gals[snap_idx].dt == NULL ||
           save_info->buffer_output_gals[snap_idx].BHAccretionType == NULL) 
           {
            ...
            free(save_info->buffer_output_gals[snap_idx].dt);
                                       ...
                                       const struct params *run_params)
{
    ...
    for(int snap = 0; snap < run_params->SimMaxSnaps; snap++) {
        ...
        save_info->buffer_output_gals[output_snap_idx].dt[idx] = g->dt[snap];
        save_info->buffer_output_gals[output_snap_idx].BHAccretionType[idx] = (int32_t)g->BHAccretionType[snap];
                             ...
                             struct save_info *save_info, const struct params *run_params)
{
    ...
    const char *bh_field_names[9] = {"BHAccretionType", "dt", "BHMassatAccretion", "BHEddingtonRateLimit", "BHMaxaccretionRate", "RadioModeBHaccretionMass", "InstabilityDrivenBHaccretionMass", "MergerDrivenBHaccretionMass", "BHMergerMass"};
    float *bh_data_ptrs[9] = {
        save_info->buffer_output_gals[snap_idx].BHAccretionType,
        save_info->buffer_output_gals[snap_idx].dt,
    ...
    for(int cum_idx = 0; cum_idx < 8; cum_idx++) {
        ...
        if(dataset_bh >= 0) {
            ...
            CHECK_STATUS_AND_RETURN_ON_FAIL(status, (int32_t) status,
                                            "Could not extend BH history dataset %s", bh_field_names[cum_idx]);
            ...
            CHECK_STATUS_AND_RETURN_ON_FAIL(status, (int32_t) status,
                                            "Could not select hyperslab for BH history dataset %s", bh_field_names[cum_idx]);

            hid_t memspace_bh = H5Screate_simple(2, count_bh, NULL);
            CHECK_STATUS_AND_RETURN_ON_FAIL(memspace_bh, (int32_t) memspace_bh,
                                            "Could not create memspace for BH history dataset %s", bh_field_names[cum_idx]);

            status = H5Dwrite(dataset_bh, H5T_NATIVE_FLOAT, memspace_bh, filespace_bh, H5P_DEFAULT, bh_data_ptrs[cum_idx]);
            CHECK_STATUS_AND_RETURN_ON_FAIL(status, (int32_t) status,
                                            "Could not write BH history dataset %s", bh_field_names[cum_idx]);
